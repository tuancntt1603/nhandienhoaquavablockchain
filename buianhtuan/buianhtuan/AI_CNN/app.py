...
def predict():
    try:
        ...
        # Chuẩn hóa ảnh (giảm giá trị pixel về khoảng 0-1)
        test_image = resized_frame / 255.0
        test_image = np.expand_dims(test_image, axis=0)

       # đưa dữ liệu (ảnh) vào mô hình đã huấn luyện và đưa dự đoán xác suất cho mỗi lớp
...
def camera_stream():
    ...
    while camera_active:
        ...
        # Chỉ giới hạn xác suất 100% cho lớp "Không phải trái cây"
        if prediction == 'Không phải trái cây' and confidence >= 99.9:
            confidence = 99.9

        # Sử dụng thư viện PIL để vẽ văn bản lên ảnh (tích hợp từ test.py)
        # Convert frame (numpy array) thành ảnh PIL để sử dụng ImageFont
        pil_img = Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
        draw = ImageDraw.Draw(pil_img)

        # Chọn phông chữ Arial in đậm và kích thước
        font = ImageFont.truetype(font_path, 32)  # Kích thước phông chữ

        # Kích thước của văn bản - không hiển thị xác suất
        text = f"Dự đoán: {prediction.upper()}"
        text_bbox = draw.textbbox((0, 0), text, font=font)
        text_width = text_bbox[2] - text_bbox[0]
        text_height = text_bbox[3] - text_bbox[1]

        # Vẽ hình chữ nhật màu trắng bao quanh văn bản
        rectangle_position = (10, 50, 10 + text_width, 50 + text_height)
        draw.rectangle(rectangle_position, fill=(255, 255, 255))

        # Vẽ văn bản lên ảnh với màu đỏ nếu không phải trái cây, màu đen nếu là trái cây
        text_color = (255, 0, 0) if prediction == 'Không phải trái cây' else (0, 0, 0)
        draw.text((10, 50), text, font=font, fill=text_color)

        # Chuyển lại ảnh về định dạng OpenCV
